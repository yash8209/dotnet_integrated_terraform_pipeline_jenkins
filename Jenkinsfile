pipeline {
    ...
    steps {
        withCredentials([azureServicePrincipal(credentialsId: AZURE_CREDENTIALS_ID)]) {
            ...
            bat "powershell Compress-Archive -Path pipeline_ex/publish/* -DestinationPath pipeline_ex/publish.zip -Force"
            bat "az webapp deploy --resource-group $RESOURCE_GROUP --name $APP_SERVICE_NAME --src-path pipeline_ex/publish.zip --type zip"
        }
    }
}
